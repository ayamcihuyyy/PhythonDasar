...
#init
window = tk.Tk() #membuat object window bernama window Tk()
...
#fungsi
def tombol_click():
    pesan = "Hello " + NAMA_DEPAN.get() + " " + NAMA_BELAKANG.get() + "! Have nice day"
    showinfo(title="Hi", message=pesan)

#frame input
input_frame = ttk.Frame(window) #menggunakan widget ttk untuk membuat frame yang akan berisi widget
input_frame.pack(padx=10, pady=10, fill="x", expand=True) #membuat layout dengan pack

#komponen
#1. label nama depan
nama_depan_label = ttk.Label(input_frame, text="Nama Depan:") #label text Alisha pada frame input
...
nama_belakang_label = ttk.Label(input_frame, text="Nama Belakang:") #label text Alisha pada frame input
